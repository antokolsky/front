...
interface Props
	...
	> {}
const MyProject: FC<Props> = (props) => {
	const navigate=useNavigate()
	const { className, ...rest } = props;
	const myProjectCn = cn(style.myProject, className);
	const positionButtonCn = cn(style.positionButton)
	const headerCn = cn(style.header)
	

	return (
		<div className={myProjectCn} {...rest}>
			<h3 className={headerCn}><Link to={PATH.MAIN.USERS.PROJECTS.PROJECTS}>Мои работы</Link></h3>
			...
			<Button variant="x2" onClick={()=>navigate(PATH.MAIN.PROJECTS.CREATPROJECT)} className={positionButtonCn}>Загрузить работу</Button>
		</div>
	);
};
